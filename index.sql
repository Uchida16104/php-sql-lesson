...
SELECT category_id,count(*) AS 'goods_count' FROM product GROUP BY category_id;
SELECT category_id,price,count(1) AS 'goods_count' FROM product GROUP BY category_id,price;
...
SELECT category_id,max(price) AS 'max'　FROM　product　GROUP BY　category_id;
SELECT category_id,min(price) AS 'min'　FROM　product　GROUP BY　category_id;
